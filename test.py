...
def plot_wind_rose_qnkt(
    ...
    """
    Vẽ hoa gió 16 hướng cho dữ liệu QNKT.xlsx, chuẩn hóa 360° = 0°.
    Pivot table và biểu đồ trùng tuyệt đối.
    """
    # --- đọc & lọc dữ liệu ---
    df = pd.read_excel(excel_path, sheet_name=sheet_name)
    df = df[df["Năm"] == year].copy()

    # Gom các cặp Hướng_i & V_i, nhân Hướng * 10
    wind_parts = []
    ...
    # --- pivot table ---
    freq = wind_df.pivot_table(index="dir_sector", columns="spd_cat", values="spd",
                               aggfunc="count", fill_value=0)
    freq = freq.reindex(labels_dir, fill_value=0)
    ...
    ax.set_theta_zero_location("N")   # Đặt góc 0° ở hướng Đông
